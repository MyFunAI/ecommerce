...
object AccuracyEvaluation extends Evaluation {
  ...
  engineMetric = (EcommerceEngine(), new Accuracy())
}
...
object EngineParamsList extends EngineParamsGenerator {
  ...
  engineParamsList = Seq(
    baseEP.copy(algorithmParamsList = Seq(("logisticSGD", AlgorithmParams(10, 0.1, 1.0)))),
    baseEP.copy(algorithmParamsList = Seq(("logisticSGD", AlgorithmParams(100, 0.01, 1.0)))),
    baseEP.copy(algorithmParamsList = Seq(("logisticSGD", AlgorithmParams(1, 0.1, 1.0)))))
}
